...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Submit Requirement</h1>
</div>
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm p-6">
        @if($projects->isEmpty())
            <div class="rounded-lg border border-yellow-300 bg-yellow-50 p-4 text-yellow-800">
                No project is assigned to you yet. Please contact the superadmin.
            </div>
        @else
            <form action="{{ route('client.requirements.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="details" id="details" rows="8" required
                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500 @error('details') border-red-500 @enderror">{{ old('details') }}</textarea>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-2 bg-gray-900 dark:bg-white hover:bg-gray-800 dark:hover:bg-gray-100 text-white dark:text-gray-900 rounded-lg transition">Submit Requirement</button>
                </div>
            </form>
        @endif
    </div>
