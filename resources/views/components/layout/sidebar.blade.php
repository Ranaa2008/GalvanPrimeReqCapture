    ...
    <nav class="p-3 space-y-0.5">
        ...
        @else
            ...
            @php
                ...
                $hasManagementAccess = $currentUser->hasAnyPermission(['view-users', 'view-roles', 'view-permissions']);
            @endphp

            @if(
                $currentUser->hasPermissionTo('create-projects') ||
                ($currentUser->hasRole('Developer') && $currentUser->hasPermissionTo('view-requirements')) ||
                ($currentUser->hasRole('Client') && $currentUser->hasPermissionTo('view-requirements'))
            )
                <div class="pt-3 pb-1 px-3">
                    <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Projects</p>
                </div>
            @endif

            @if($currentUser->hasPermissionTo('create-projects'))
                <a href="{{ route('projects.create') }}" class="flex items-center space-x-2 px-3 py-2 text-sm rounded-lg {{ request()->routeIs('projects.create') ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900' : 'text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-900' }} transition" @click="sidebarOpen = false">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                    </svg>
                    <span class="font-medium">Create Projects</span>
                </a>
            @endif

            @if($currentUser->hasPermissionTo('view-projects'))
                <a href="{{ route('projects.index') }}" class="flex items-center space-x-2 px-3 py-2 text-sm rounded-lg {{ request()->routeIs('projects.index') || request()->routeIs('projects.edit') ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900' : 'text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-900' }} transition" @click="sidebarOpen = false">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if($currentUser->hasRole('Developer') && $currentUser->hasPermissionTo('view-requirements'))
                <a href="{{ route('projects.my') }}" class="flex items-center space-x-2 px-3 py-2 text-sm rounded-lg {{ request()->routeIs('projects.my') || request()->routeIs('projects.show') ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900' : 'text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-900' }} transition" @click="sidebarOpen = false">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if($currentUser->hasRole('Client') && $currentUser->hasPermissionTo('view-requirements'))
                <a href="{{ route('requirements.my') }}" class="flex items-center space-x-2 px-3 py-2 text-sm rounded-lg {{ request()->routeIs('requirements.my') || request()->routeIs('requirements.create') || request()->routeIs('requirements.edit') || request()->routeIs('requirements.show') ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900' : 'text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-900' }} transition" @click="sidebarOpen = false">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3h6v4H9z"></path>
                    </svg>
