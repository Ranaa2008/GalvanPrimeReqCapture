...
<div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-6">
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Phone Verification</h3>
    ...
    <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
        Phone: <strong class="text-gray-900 dark:text-white">{{ $user->phone_number }}</strong>
    </p>

    @if(!$user->phone_verified)
        <div x-data="{ 
            countdown: {{ session('phone_otp_sent') || $errors->has('otp_code') ? 300 : 0 }},
            canSend: {{ session('phone_otp_sent') || $errors->has('otp_code') ? 'false' : 'true' }},
            init() {
                if (!this.canSend) {
                    this.startCountdown();
                }
            ...
            <!-- Send OTP Form -->
            <form method="POST" action="{{ route('profile.send-phone-otp') }}" @submit="startCountdown()" x-show="canSend">
                @csrf
            ...
            <div x-show="!canSend" class="text-center">
                ...
                <!-- Verify OTP Form -->
                <form method="POST" action="{{ route('profile.verify-phone-otp') }}" class="space-y-3">
                    @csrf
                    <input 
                        type="text" 
                        name="otp_code" 
    ...
    @else
        <p class="text-sm text-green-600 dark:text-green-400">Your phone has been verified successfully.</p>
    @endif
</div>
