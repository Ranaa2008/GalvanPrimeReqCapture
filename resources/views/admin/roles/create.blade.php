        ...
        <form action="{{ route('admin.roles.store') }}" method="POST" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="number" name="superiority" id="superiority" value="{{ old('superiority', 99) }}" 
                    ...
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        Lower number = Higher authority. Level 1 is reserved for Super Admin (only 1 person). Level 2 for System Owner. Start from 3+
                    </p>
                    @error('superiority')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
