    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden border border-gray-200 dark:border-gray-700">
        ...
        <form action="{{ route('admin.users.update', $user) }}" method="POST" class="p-6">
            ...
            <div class="mb-6">
                ...
                <input type="tel" name="phone_number_secondary" id="phone_number_secondary" value="{{ old('phone_number_secondary', $user->phone_number_secondary) }}" 
            ...
            <div class="mb-6">
                ...
                <textarea name="address" id="address" rows="3" 
                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500 @error('address') border-red-500 @enderror">{{ old('address', $user->address) }}</textarea>
                @error('address')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">New Password (leave empty to keep current)</label>
                    <input type="password" name="password" id="password" 
                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500 @error('password') border-red-500 @enderror">
            ...
            <div class="mb-6" 
                x-data="{
                    ...
                    currentManager: {{ old('managed_by', $user->managed_by) ?? 'null' }},
                    
                    init() {
                        console.log('Manager selector initialized (Edit Mode)');
                        console.log('Available roles:', this.rolesData);
                        console.log('Current manager:', this.currentManager);
                        
                        // Initial load based on current roles
                        this.updateManagers();
                ...
                <select name="managed_by" id="managed_by" x-ref="managerSelect"
                    ...
                    <template x-for="manager in availableManagers" :key="manager.id">
                        <option :value="manager.id" :selected="manager.id == currentManager" x-text="`${manager.name} (${manager.email}) - ${manager.roles}`"></option>
                    </template>
                </select>
            ...
            <div class="flex justify-end space-x-3">
                ...
                <button type="submit" id="updateUserBtn" class="px-6 py-2 bg-gray-900 dark:bg-white hover:bg-gray-800 dark:hover:bg-gray-100 text-white dark:text-gray-900 rounded-lg transition">
                    Update User
                </button>
            </div>
        </form>
