        ...
        <form action="{{ route('admin.projects.update', $project) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $project->description) }}</textarea>
            ...
            <div>
                ...
                <select name="client_ids[]" id="client_ids" multiple size="6"
                    ...
                    @foreach($clients as $client)
                        <option value="{{ $client->id }}" @selected(in_array($client->id, old('client_ids', $selectedClientIds)))>
            ...
            <div>
                ...
                <select name="developer_ids[]" id="developer_ids" multiple size="6"
                    ...
                    @foreach($developers as $developer)
                        <option value="{{ $developer->id }}" @selected(in_array($developer->id, old('developer_ids', $selectedDeveloperIds)))>
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2 bg-gray-900 dark:bg-white hover:bg-gray-800 dark:hover:bg-gray-100 text-white dark:text-gray-900 rounded-lg transition">Update Project</button>
            </div>
        </form>
    </div>
</div>
